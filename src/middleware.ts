...
export async function middleware(request: NextRequest) {
  ...
  if (pathname.startsWith("/dashboard")) {
    ...
    if (matchesDashboardRole(pathname, "admin") && session.userType !== "ADMIN") {
      return NextResponse.redirect(new URL(getDashboardPathForRole(session.userType), request.url));
    }

    if (
      matchesDashboardRole(pathname, "administrator") &&
      session.userType !== "ADMINISTRATION"
    ) {
      return NextResponse.redirect(new URL(getDashboardPathForRole(session.userType), request.url));
    }

    if (matchesDashboardRole(pathname, "owner") && session.userType !== "OWNER") {
      return NextResponse.redirect(new URL(getDashboardPathForRole(session.userType), request.url));
    }
  }
