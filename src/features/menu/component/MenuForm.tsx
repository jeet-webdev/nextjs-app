...
export default function MenuForm({
  ...
  /// 2
  const [showItems, setShowItems] = useState<MenuRecord[]>([]); //MenuItemDisplay

  const [showMenuItamDisplay, setShowMenuItamDisplay] = useState(false);
  ...
  useEffect(() => {
    if (lockedCategoryId) {
      ...
      setRows((prev) =>
        prev.map((r) =>
          r.categoryId ? r : { ...r, categoryId: categories[0].id },
        ),
      );
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEditing) {
        ...
        const saved =
          data?.menuItems ?? (data?.menuItem ? [data.menuItem] : []);
 ...
 <h3 className="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-3">
        ...
        <button
          onClick={() => setShowMenuItamDisplay(!showMenuItamDisplay)}
        ...
        >
          {showMenuItamDisplay ? (
            <>
              <EyeOff size={18} /> Hide Items
            </>
          ) : (
            <>
              <Eye size={18} /> Show Items ({showItems.length})
            </>
          )}
        </button>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4">
            {showItems.length > 0 ? (
              showItems.map((item) => (
                <div
                  key={item.id}
                  className="border flex flex-row justify-between p-4 rounded-lg shadow-sm"
                >
                  <div className="flex flex-col justify-start">
                    ...
                    <span
                      className={`ml-2 w-fit px-1.5 py-0.5 mt-8 rounded text-xs ${
                        item.isAvailable
                  ...
                  <div className="flex items-center flex-col justify-end gap-1.5">
                    <button
                      type="button"
                    ...
                    <button className="flex items-center gap-1.5 rounded-lg border border-sky-500/30 bg-sky-500/10 text-sky-400 hover:bg-sky-500/20 hover:text-sky-300 px-3 py-1.5 text-xs font-semibold transition">
                      Add Modifier
                    </button>
                  </div>
                </div>
